...
export default function VideoEditor() {
  ...
  return (
    <div className="bg-purple-50 p-6 rounded-lg w-full max-w-4xl mx-auto">
              ...
              <div key={step.id} className="flex flex-col items-center">
                <button
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    currentStep === step.id ? "bg-purple-400 text-white" : "bg-purple-200 text-purple-600"
                  } mb-2`}
                >
                  {step.id}
                </button>
            ...
            {steps.map((step, index) => (
              <React.Fragment key={`line-${step.id}`}>
                ...
                {index < steps.length - 1 && (
                  <div className={`h-0.5 flex-1 ${index < currentStep - 1 ? "bg-purple-400" : "bg-purple-200"}`}></div>
                )}
              </React.Fragment>
            ))}
          </div>
      ...
      <div className="mb-4">
        <h3 className="text-purple-600 mb-4">{steps[currentStep - 1].name}</h3>
        <div className="bg-white p-4 rounded-lg">{steps[currentStep - 1].component({})}</div>
      </div>
